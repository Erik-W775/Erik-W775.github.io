//header file for most commonly used math operations for matrices manipulation and world view movvement and creation
...
#endif
//define vector size for matrices calculations
...
LINMATH_H_FUNC void vec##n##_scale(vec##n r, vec##n const v, float const s) \ //scaling for vector size
{ \
	int i; \
	for(i=0; i<n; ++i) \
		r[i] = v[i] * s; \
...
LINMATH_H_FUNC void vec3_reflect(vec3 r, vec3 const v, vec3 const n)//defines 3 value vector
{
...
LINMATH_H_FUNC void vec4_mul_cross(vec4 r, vec4 a, vec4 b)//4 vector cross matrices calculations 
{
	r[0] = a[1] * b[2] - a[2] * b[1];
	r[1] = a[2] * b[0] - a[0] * b[2];
	r[2] = a[0] * b[1] - a[1] * b[0];
